{{--
    Ожидает: $initialCharacters — массив [{ id, character, pinyin, meaning, hsk_level }, ...]
--}}
@php
...
<div
    ...
    data-search-url="{{ route('admin.builtin-collections.characters.search') }}"
    data-import-url="{{ route('admin.builtin-collections.glyphs-json-import') }}"
    data-resolve-url="{{ route('admin.builtin-collections.characters.resolve') }}"
>
    <section class="collections-panel collections-panel--accent">
        ...
        {{-- CSS (.page-collections-show .suggest) задаёт display:none — показывать только через style.display, как на странице коллекции пользователя --}}
    ...
    <input type="hidden" name="character_ids" id="builtinGlyphCharacterIds" value="{{ $characterIdsValue }}">
</div>
...
(function() {
    ...
    const initialRows = @json($initialCharacters ?? []);
