    ...
    @forelse($articles as $article)
        <article class="article-card">
            ...
            <div class="article-card-body">
                ...
                <p class="article-card-excerpt">{{ \Illuminate\Support\Str::limit(trim(preg_replace('/\s+/u', ' ', strip_tags($article->content))), 400) }}</p>
                ...
                <div class="article-card-actions">
                    ...
                    <form method="POST" action="{{ route('articles.favorite.toggle', $article) }}">
                        ...
                        <button
                            type="submit"
                            class="favorite-icon-btn {{ in_array($article->id, $favoriteIds, true) ? 'is-active' : '' }}"
                            aria-label="{{ in_array($article->id, $favoriteIds, true) ? 'Убрать из избранного' : 'Добавить в избранное' }}"
                            title="{{ in_array($article->id, $favoriteIds, true) ? 'Убрать из избранного' : 'Добавить в избранное' }}"
                        >
